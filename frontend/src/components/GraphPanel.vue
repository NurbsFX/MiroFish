...
const renderGraph = () => {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to keep isolated node clusters near the center
  ...
  const getLinkPath = (d) => {
    ...
    // More edges result in a larger offset-to-distance ratio
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, each additional edge adds 5%
