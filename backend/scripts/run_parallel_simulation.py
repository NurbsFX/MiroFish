...
def disable_oasis_logging():
    ...
    OASIS logs are too noisy (logging every agent's observations and actions); we use our own action_logger instead.
    """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None/unspecified: Interview on both platforms and return combined results
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    """
    Get the agent_id -> entity_name mapping from the simulation config.

    This allows actions.jsonl to display actual entity names instead of generic labels like "Agent_0".
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: The highest rowid from the last read (using rowid instead of created_at because different platforms use different created_at formats)
        agent_names: Mapping of agent_id to agent_name

    Returns:
        (actions_list, new_last_rowid)
        - actions_list: List of actions, each containing agent_id, agent_name, action_type, action_args (with context)
        - new_last_rowid: The new highest rowid value
    """
    actions = []
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM configurations to speed up parallel simulations:
    ...
    for different platforms, increasing concurrency.
...
async def run_twitter_simulation(
    ...
    # For agents not in config, use OASIS default names
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM configuration (if available, otherwise falls back to standard)
