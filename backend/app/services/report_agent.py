...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Log section content generation complete (only records content, does not mean the entire section is done)"""
        self.log(
    ...
    def log_section_full_complete(
        ...
        The frontend should listen for this log entry to determine if a section is truly complete and get the full content
        """
        self.log(
...
This tool is used to get a complete overview of simulation results, particularly suitable for understanding event evolution. It will:
...
This is not an LLM simulation, but calls the actual interview interface to get simulation Agents' original responses.
...
[Core Concept]
We have built a simulated world and injected specific "simulation requirements" as variables. The simulation's evolutionary results represent predictions of what might happen in the future. What you are observing is not "experimental data," but a "rehearsal of the future."
...
DO focus on "what the future would look like" — the simulation results ARE the predicted future.
...
1. [Must invoke tools to observe the simulated world]
   ...
   - Each section must invoke tools at least 3 times (maximum 5) to observe the simulated world, which represents the future
...
3. [Language consistency - quoted content must match the report language]
   ...
   - When quoting content from tools, translate it into the report's language for fluency
...
The platform served as the primary scene for public sentiment, taking on the core function of initial information dissemination:
...
- Each response may invoke at most one tool
...
4. [Quote format specification - must be a standalone paragraph]
   ...
   Correct format:
   ```
   The response was considered to lack substantive content.
   ...
   Incorrect format:
   ```
   The response was considered to lack substantive content. > "The response pattern..." This assessment reflects...
   ```
...
2. Then invoke tools (Action) to retrieve simulation data
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Notice] You have only invoked tools {tool_calls_count} times, at least {min_tool_calls} times required. "
    "Please invoke more tools to retrieve additional simulation data before outputting Final Answer. {unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Currently only {tool_calls_count} tool invocations made, at least {min_tool_calls} required. "
...
[Available Tools] (use only when needed, maximum 1-2 invocations)
{tools_description}
...
class ReportAgent:
    ...
    2. Generation phase: Generate content section by section, each section can invoke tools multiple times
    ...
    # Maximum tool invocations in chat
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # In-depth interview - call the real OASIS interview API to get simulation Agents' responses (dual platform)
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format 2: Fallback - LLM directly output bare JSON (without <tool_call> tags)
        # Only attempted when Format 1 didn't match, to avoid false matches on JSON in body text
    ...
    def _generate_section_react(
        ...
        2. Action - Invoke tools to get information
        ...
        # Build user prompt - pass in max 4000 characters from each completed section
        ...
        used_tools = set()  # Track tool names that have been invoked
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None, break out of loop into forced finalization
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        "role": "user",
                        "content": (
                            "[Format Error] Your response contained both a tool call and Final Answer simultaneously, which is not allowed.\n"
            ...
            if has_tool_calls:
                ...
                call = tool_calls[0]
                if len(tool_calls) > 1:
                    logger.info(f"LLM attempted to invoke {len(tool_calls)} tools, only executing the first: {call['name']}")
            ...
            # Tool invocations sufficient, LLM output content without "Final Answer:" prefix
            # Directly accept this content as the final answer, no more idle iterations
            logger.info(f"Section {section.title}: 'Final Answer:' prefix not detected, directly adopting LLM output as final content (tool invocations: {tool_calls_count})")
    ...
    def generate_report(
        ...
        Each section is saved to the folder immediately upon completion, no need to wait for the entire report.
        File structure:
        reports/{report_id}/
    ...
    def chat(
        ...
        During chat, Agent can autonomously invoke retrieval tools to answer questions
