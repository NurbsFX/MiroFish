...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: A fallback type for any individual. When a person does not fit any other more specific person type, they fall into this category.
   - `Organization`: A fallback type for any organization. When an organization does not fit any other more specific organization type, it falls into this category.

B. **Specific types (8, designed based on text content)**:
   - Design more specific types for the main roles appearing in the text
   - For example: if the text involves academic events, you could have `Student`, `Professor`, `University`
   - For example: if the text involves business events, you could have `Company`, `CEO`, `Employee`

**Why fallback types are needed**:
- Various people appear in the text, such as "elementary school teacher", "bystander", "anonymous netizen"
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # If text exceeds 50,000 characters, truncate (only affects what is sent to the LLM, not graph construction)
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            combined_text = combined_text[:self.MAX_TEXT_LENGTH_FOR_LLM]
            combined_text += f"\n\n...(Original text is {original_length} characters; the first {self.MAX_TEXT_LENGTH_FOR_LLM} characters were used for ontology analysis)..."
