...
Converts entities from Zep graphs into the Agent Profile format required by the OASIS simulation platform

Optimizations:
1. Uses Zep search to enrich node information with additional context
...
class OasisProfileGenerator:
    ...
    Converts entities from Zep graphs into Agent Profiles for OASIS simulation

    Optimized features:
    1. Uses Zep graph search to retrieve richer context
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search endpoint; edges and nodes
        must be searched separately, then results merged.
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        Includes:
        1. Edge information (facts) associated with the entity
        ...
        if zep_results.get("facts"):
            ...
            if new_facts:
                context_parts.append("### Facts Retrieved via Zep Search\n" + "\n".join(f"- {f}" for f in new_facts[:15]))

        if zep_results.get("node_summaries"):
            context_parts.append("### Related Nodes Retrieved via Zep Search\n" + "\n".join(f"- {s}" for s in zep_results["node_summaries"][:10]))
    ...
    def _generate_profile_with_llm(
        ...
        # Try generating multiple times until successful or max retries reached
        max_attempts = 3
        last_error = None
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for opinion simulation, faithfully reconstructing existing real-world circumstances as much as possible. You must return valid JSON format; all string values must not contain unescaped newlines. Use Chinese."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, faithfully reconstructing existing real-world circumstances as much as possible.
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Personal memory (an important part of the persona; describe this individual's connection to the event, and their existing actions and reactions within the event)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this institution/group entity, faithfully reconstructing existing real-world circumstances as much as possible.
...
2. persona: Detailed account profile description (2000 characters of plain text), must include:
   ...
   - Institutional memory (an important part of the persona; describe this institution's connection to the event, and its existing actions and reactions within the event)
...
- Institutional account communications must be consistent with its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID, used for Zep search to retrieve richer context
        ...
        completed_count = [0]  # Use a list to allow modification in closure
        lock = Lock()
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - user_id: User ID (starts from 0 based on CSV row order)
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Must include user_id field, which is the key for matching in OASIS agent_graph.get_agent()!
