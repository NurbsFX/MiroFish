...
class PlatformConfig:
    ...
    # Viral spread threshold (number of interactions needed to trigger viral spread)
...
class SimulationConfigGenerator:
    ...
    then automatically generates optimal simulation parameter configurations.
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # Reserve 500 characters as buffer
...
- The user base is Chinese, so the schedule must follow Beijing Time daily routines
- Almost no activity from midnight to 5 AM (activity multiplier 0.05)
- Gradually active from 6-8 AM (activity multiplier 0.4)
- Moderate activity during work hours 9 AM-6 PM (activity multiplier 0.7)
- Evening peak from 7-10 PM (activity multiplier 1.5)
- Activity declines after 11 PM (activity multiplier 0.5)
- General pattern: low activity in early morning, gradually increasing in the morning, moderate during work hours, peak in the evening
- **Important**: The example values below are for reference only; you should adjust specific time periods based on the nature of the event and characteristics of participants
  - For example: student groups may peak at 9-11 PM; media may be active all day; government agencies only during work hours
    ...
    def _assign_initial_post_agents(
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                ...
                if agent_configs:
                    # Sort by influence and select the most influential
...
- **Schedule must follow Chinese daily routines**: almost no activity from midnight to 5 AM, most active from 7-10 PM
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours, following Chinese daily routines>],
